...
def write_prefab(prefab_path: str, name: str,
    ...
    # Stable fileIDs — use high values to avoid Unity class-ID collisions.
    root_go = 400000
    root_tf = 400001
    mf = 400002
    mr = 400003
    ...
    for i in range(len(collider_guids)):
        child_go_ids.append(500000 + i * 10)
        child_tf_ids.append(500001 + i * 10)
...
# ---------------------------------------------------------------------------
# Final "READY" prefab: all LODs as children + LODGroup + shared materials +
# compound colliders (LOD0 only) + Rigidbody + S4Identity.  Generated entirely
# in Python so no Unity "S4 - Tools - Fix" step is required.
# ---------------------------------------------------------------------------
def _write_script_meta(script_path: str, guid: str) -> None:
    with open(script_path + ".meta", "w", encoding="utf-8") as f:
        f.write(
            "fileFormatVersion: 2\n"
            f"guid: {guid}\n"
            "MonoImporter:\n"
            ...
            "  executionOrder: 0\n"
            "  isPreProcessed: 0\n"
            "  isWarningPreProcessed: 0\n"
            "  userData: \n"
            "  assetBundleName: \n"
            "  assetBundleVariant: \n")


def write_s4identity_script(out_root: str, id_str: str = "", asset_name: str = "",
                             shared_scripts_dir: str | None = None) -> str:
    """Write S4Identity.cs (+ .meta with a deterministic GUID) once.

    If *shared_scripts_dir* is given the script is placed there (one copy for
    all exported packages) instead of inside *out_root*.  When the file
    already exists the write is skipped so multiple packages never create
    duplicate class definitions that conflict inside one Unity project.
    """
    # Determine where to write the script.
    script_dir_root = shared_scripts_dir if shared_scripts_dir else out_root
    runtime_dir = os.path.join(script_dir_root, "Runtime")
    script_path = os.path.join(runtime_dir, "S4Identity.cs")

    # GUID is always derived from the shared path so every prefab that
    # references this script gets the same GUID regardless of which
    # package's out_root triggered the call.
    guid_key = os.path.join(script_dir_root, "Runtime", "S4Identity.cs")
    guid = _guid_for(guid_key)

    # Skip if the script was already written (prevents duplicates).
    ...
    with open(script_path, "w", encoding="utf-8") as f:
        f.write(cs)
    _write_script_meta(script_path, guid)
    return guid


def write_ready_prefab(prefab_path: str, name: str,
                       lod_mesh_specs: list[dict],
                       collider_guids: list[str],
                       lod0_bbox: tuple | None = None,
                       dynamic: bool = True,
                       identity: tuple | None = None,
                       identity_script_guid: str | None = None) -> None:
    """Write a ready-to-use prefab with a LODGroup.

    lod_mesh_specs: list of dicts with keys mesh_name, fbx_guid, material_guid,
                    lod_index (>=0).  Each spec becomes a child GameObject with a
                    MeshFilter (FBX mesh) + MeshRenderer (shared material).
    collider_guids: list of collider-mesh GUIDs (built from LOD0 only).  They are
                    added as MeshCollider components on the root Rigidbody.
    lod0_bbox: (minx,miny,minz,maxx,maxy,maxz) of LOD0, used for LODGroup size.
    """
    guid = _guid_for(os.path.basename(prefab_path))

    # Use high fileIDs to avoid collisions with Unity's internal class-ID
    # mapping (e.g. 100002 is interpreted as RenderSettings by Unity 6).
    root_go      = 400000
    root_tf      = 400001
    lodgroup     = 400002
    rb           = 400003
    identity_id  = 400004
    collider_base = 400010

    child_ids = []  # (go, tf, mf, mr, spec)
    for i, spec in enumerate(lod_mesh_specs):
        go = 500000 + i * 10
        tf = 500001 + i * 10
        mf = 500002 + i * 10
        mr = 500003 + i * 10
        child_ids.append((go, tf, mf, mr, spec))

    # Group renderers by LOD index for the LODGroup.
    lod_renderers: dict[int, list[int]] = {}
    for (go, tf, mf, mr, spec) in child_ids:
        li = spec.get("lod_index", -1)
        if li < 0:
            continue
        lod_renderers.setdefault(li, []).append(mr)
    lod_keys = sorted(lod_renderers.keys())
    screen_heights = [0.25, 0.15, 0.05, 0.01]
    lods = []
    for idx_i, k in enumerate(lod_keys):
        height = (0.0 if idx_i == len(lod_keys) - 1
                  else (screen_heights[idx_i] if idx_i < len(screen_heights) else 0.01))
        lods.append((height, lod_renderers[k]))
    ...
    if lod0_bbox:
        ...
        cy = (mn[1] + mx[1]) / 2.0
        cz = (mn[2] + mx[2]) / 2.0
        dx = max(abs(mx[0] - cx), abs(mn[0] - cx))
        dy = max(abs(mx[1] - cy), abs(mn[1] - cy))
        dz = max(abs(mx[2] - cz), abs(mn[2] - cz))
        radius = math.sqrt(dx * dx + dy * dy + dz * dz)
        size = 2.0 * radius if radius > 1e-6 else 1.0
    else:
        cx = cy = cz = 0.0
        size = 1.0

    L: list[str] = ["%YAML 1.1", "%TAG !u! tag:unity3d.com,2011:"]

    # Root GameObject
    comps = [root_tf, lodgroup]
    if dynamic:
        comps.append(rb)
    if identity_script_guid:
        comps.append(identity_id)
    for i in range(len(collider_guids)):
        comps.append(collider_base + i)
    ...
    L.append("  m_Name: ")
    L.append("  serializedVersion: 2")
    L.append(f"  m_LocalReferencePoint: {{x: {cx:.6f}, y: {cy:.6f}, z: {cz:.6f}}}")
    L.append(f"  m_Size: {size:.6f}")
    L.append(f"  m_Center: {{x: {cx:.6f}, y: {cy:.6f}, z: {cz:.6f}}}")
    L.append("  m_Enabled: 1")
    L.append("  m_LODs:")
    for height, renderers in lods:
        L.append("  - first:")
        L.append(f"      m_ScreenRelativeHeight: {height:.6f}")
        L.append("      m_Renderers:")
        for r in renderers:
            L.append(f"      - {{fileID: {r}}}")
        if not renderers:
            L[-1] = "      m_Renderers: []"
        L.append("    second:")
    L.append("  m_Renderers: []")
    L.append("  m_FadeMode: 0")
    L.append("  m_AnimateCrossFading: 0")
    L.append("  m_FadeTransitionWidth: 0.5")
    ...
    if identity_script_guid:
        ...
        L.append(f"  m_Script: {{fileID: 11500000, guid: {identity_script_guid}, type: 3}}")
        L.append("  m_Name: ")
        L.append("  m_EditorClassIdentifier: ")
        id_val = identity[0] if identity else 0
        asset_val = identity[1] if identity else ""
        L.append(f"  id: {id_val}")
        L.append(f"  assetName: {asset_val}")

    # Colliders (compound) on the root Rigidbody
    for i, cguid in enumerate(collider_guids):
        cid = collider_base + i
        L.append(f"--- !u!64 &{cid}")
        L.append("MeshCollider:")
    ...
    for (go, tf, mf, mr, spec) in child_ids:
        ...
        L.append("  serializedVersion: 6")
        L.append("  m_Component:")
        L.append(f"  - component: {{fileID: {tf}}}")
        L.append(f"  - component: {{fileID: {mf}}}")
        L.append(f"  - component: {{fileID: {mr}}}")
        L.append("  m_Layer: 0")
        L.append(f"  m_Name: {spec['mesh_name']}")
        L.append("  m_TagString: Untagged")
        L.append("  m_IsActive: 1")

        L.append(f"--- !u!4 &{tf}")
        L.append("Transform:")
        L.append(f"  m_GameObject: {{fileID: {go}}}")
        L.append("  serializedVersion: 2")
        L.append("  m_LocalRotation: {x: 0, y: 0, z: 0, w: 1}")
        L.append("  m_LocalPosition: {x: 0, y: 0, z: 0}")
        L.append("  m_LocalScale: {x: 1, y: 1, z: 1}")
        ...
        L.append("MeshFilter:")
        L.append(f"  m_GameObject: {{fileID: {go}}}")
        L.append(f"  m_Mesh: {{fileID: 4300000, guid: {spec['fbx_guid']}, type: 3}}")
        ...
        L.append("  m_ReceiveShadows: 1")
        L.append("  m_Materials:")
        mg = spec.get("material_guid")
        if mg:
            L.append(f"  - {{fileID: 2100000, guid: {mg}, type: 2}}")
        else:
            L.append("  - {fileID: 0}")
...
{break_entries_src}
    ...
    {{
        ...
        {{
            ...
            float[] screenHeights = new float[] {{ 0.25f, 0.15f, 0.05f, 0.01f }}; // порог (доля высоты экрана), при котором LOD_i уступает следующему: LOD0->LOD1 при 25%
...
def write_batch_editor_script(parent_dir: str, pipeline: str = "builtin") -> str:
    ...
    write_breakable_runtime_script(parent_dir, parent_dir)
...
{{
    ...
    {{
        ...
        {{
            ...
            {{
                ...
                string partsPrefab = CreatePartsPrefab(exportData, jsonFile);
                
                totalFixed += fixedCount;
                totalAssigned += assigned;
                if (!string.IsNullOrEmpty(readyPrefab)) totalPrefabs++;
                if (!string.IsNullOrEmpty(partsPrefab)) totalPrefabs++;
            }}
            catch (System.Exception e)
            {{
                Debug.LogError($"s4extract: Failed to process {{Path.GetFileName(jsonFile)}}: {{e.Message}}");
            }}
        }}
    ...
    {{
        ...
        {{
            ...
            string partsPrefab = CreatePartsPrefab(exportData, lastFile);
    ...
    {{
        ...
        public PartAssetEntry[] partAssets;
        public BreakSpecEntry[] breakSpecs;
    }}
    ...
    static string CreatePartsPrefab(ExportData data, string jsonPath)
    {{
        if (data.partAssets.Length == 0) return "";
        
        Material first = data.materials.Length > 0 ? FindMaterial(data.materials[0].materialName) : null;
        if (first == null) return "";
        ...
        string folder = Path.GetDirectoryName(matPath).Replace("\\\\", "/");
        string prefabPath = folder + "/" + data.assetName + "_PARTS.prefab";

        GameObject root = new GameObject(data.assetName + "_PARTS");
        int added = 0;
        
        foreach (PartAssetEntry pa in data.partAssets)
        {{
            GameObject model = FindModelAsset(pa.assetName);
            if (model == null) continue;
            Material target = FindMaterial(pa.materialName) ?? first;
            ...
            BreakSpecEntry spec = null;
            foreach (BreakSpecEntry bs in data.breakSpecs)
            {{
                if (bs.intactAssetName == pa.assetName)
                {{
                    spec = bs;
                    break;
                }}
            ...
            var brokenRoots = new List<GameObject>();
            if (spec != null && spec.brokenAssetNames != null)
            {{
                for (int i = 0; i < spec.brokenAssetNames.Length; i++)
                {{
                    string brokenAssetName = spec.brokenAssetNames[i];
