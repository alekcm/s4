...
def write_s4identity_script(out_root: str, id_str: str = "", asset_name: str = "") -> str:
    """Write S4Identity.cs (+ .meta with a per-export-unique GUID) once.

    The GUID is derived from the export folder so two different packages never
    collide on the same MonoScript GUID inside one Unity project.
    """
    ...
    with open(script_path, "w", encoding="utf-8") as f:
        f.write(cs)
    guid = _guid_for(os.path.join(out_root, "Runtime", "S4Identity.cs"))
